...
async function fetchPing(prefix,game){const grid=document.getElementById(prefix+'-gm-preset-grid');if(!grid)return;const items=grid.querySelectorAll('.gm-preset-item');const lastItem=items[items.length-1];if(lastItem){lastItem.style.opacity='0.5';lastItem.querySelector('b').nextSibling.textContent=' در حال اندازه‌گیری...';}try{const r=await authF('/api/ping/game/'+game);if(!r.ok)return;const d=await r.json();if(d.best&&d.avg_ping){if(lastItem){lastItem.style.opacity='1';lastItem.querySelector('b').nextSibling.textContent=' '+Math.round(d.avg_ping)+'ms';}const banner=document.getElementById(prefix+'-gm-body');if(banner){const sbLines=banner.querySelectorAll('.gm-sb-line');if(sbLines[1])sbLines[1].innerHTML='<i class="ti ti-world"></i> <b>بهترین سرور:</b> '+d.best.name;if(sbLines[2])sbLines[2].innerHTML='<i class="ti ti-clock"></i> <b>پینگ واقعی:</b> '+Math.round(d.avg_ping)+'ms';}}}catch(e){if(lastItem){lastItem.style.opacity='1';lastItem.querySelector('b').nextSibling.textContent=' خطا';}}}
