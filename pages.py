...
document.getElementById('nl-bwsaver').addEventListener('change',function(){const lbl=this.closest('label');const knob=lbl.querySelector('.bwsaver-knob');if(this.checked){if(knob){knob.style.transform='translateX(20px)';knob.style.background='var(--success-t)'}lbl.querySelector('span:nth-child(2)').style.background='rgba(0,255,136,.2)';lbl.querySelector('span:nth-child(2)').style.borderColor='rgba(0,255,136,.4)'}else{if(knob){knob.style.transform='translateX(0)';knob.style.background='var(--t2)'}lbl.querySelector('span:nth-child(2)').style.background='rgba(255,255,255,.1)';lbl.querySelector('span:nth-child(2)').style.borderColor='var(--glass-b)'}});
</script></body></html>"""
...
<div class="modal-bg" id="modal-edit-link">
  <div class="modal">
    ...
    <div style="margin-top:16px"><div class="cp-block mb16"><div class="cp-block-label"><i class="ti ti-category"></i> دسته‌بندی</div><select class="cp-input-full fs" id="el-category"><option value="browsing">🌐 مرورگر</option><option value="streaming">📺 استریمینگ</option><option value="social">💬 شبکه اجتماعی</option><option value="download">📥 دانلود</option><option value="vip">⭐ VIP</option><option value="economy">💰 اقتصادی</option></select></div><div class="cp-block"><div class="cp-block-label"><i class="ti ti-device-floppy"></i> کاهش مصرف</div><label class="tog" style="width:44px;height:24px;border-radius:12px"><input type="checkbox" id="el-bwsaver" style="opacity:0;width:0;height:0"><span style="position:absolute;inset:0;background:rgba(255,255,255,.1);border:1.5px solid var(--glass-b);border-radius:12px;cursor:pointer;transition:.3s"></span></label><div style="font-size:11px;color:var(--t3);margin-top:6px">فشرده‌سازی Brotli — کاهش ۱۵-۴۰٪ مصرف ترافیک</div></div></div>
...
<section class="pg" id="pg-links">
  ...
  <div class="cp">
    ...
    <div class="cp-body">
      ...
      <div class="cp-block mb16"><div class="cp-block-label"><i class="ti ti-device-floppy"></i> کاهش مصرف پهنای باند</div><label style="display:inline-block;position:relative;width:44px;height:24px;cursor:pointer"><input type="checkbox" id="nl-bwsaver" style="opacity:0;width:0;height:0;position:absolute"><span style="position:absolute;width:18px;height:18px;left:2px;bottom:2px;background:var(--t2);border-radius:50%;transition:.3s" class="bwsaver-knob"></span><span style="position:absolute;inset:0;background:rgba(255,255,255,.1);border:1.5px solid var(--glass-b);border-radius:12px;cursor:pointer;transition:.3s"></span></label><div style="font-size:11px;color:var(--t3);margin-top:6px">فشرده‌سازی Brotli — کاهش ۱۵-۴۰٪ مصرف ترافیک</div></div>
...
async function createLink(){const label=document.getElementById('nl-label').value.trim()||'کانفیگ جدید';const val=document.getElementById('nl-val').value;const unit=document.getElementById('nl-unit').value;const exp=document.getElementById('nl-exp').value;const note=document.getElementById('nl-note').value.trim();const protocol=document.getElementById('nl-proto').value||'vless-ws';const fingerprint=document.getElementById('nl-fp').value||'chrome';const alpn=document.getElementById('nl-alpn').value.trim();const port=443;const ip_limit=Number(document.getElementById('nl-iplimit').value)||0;const speed_limit_value=Number(document.getElementById('nl-speed').value)||0;const speed_limit_unit=document.getElementById('nl-speed-unit').value;const category=document.getElementById('nl-category').value||'browsing';const bandwidth_saver=document.getElementById('nl-bwsaver').checked;try{const r=await authF('/api/links',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({label,limit_value:val||0,limit_unit:unit,expires_days:exp||0,note,protocol,fingerprint,alpn,port,ip_limit,speed_limit_value,speed_limit_unit,category,bandwidth_saver})});if(!r.ok)throw new Error('failed');['nl-label','nl-val','nl-exp','nl-note','nl-alpn'].forEach(id=>document.getElementById(id).value='');document.getElementById('nl-iplimit').value='0';document.getElementById('nl-speed').value='0';document.getElementById('nl-alpn-preset').value='';document.getElementById('nl-alpn').style.display='none';document.getElementById('nl-bwsaver').checked=false;document.getElementById('nl-category').value='browsing';toast('کانفیگ ساخته شد ✓','ok');loadLinks();}catch(e){toast('خطا در ساخت','err')}}
function openEditLink(uuid){const l=allLinksList.find(x=>x.uuid===uuid);if(!l)return;document.getElementById('el-uuid').value=uuid;document.getElementById('el-label').value=l.label;document.getElementById('el-note').value=l.note||'';if(l.limit_bytes===0){document.getElementById('el-val').value='';document.getElementById('el-unit').value='GB';}else{document.getElementById('el-val').value=(l.limit_bytes/1024/1024).toFixed(0);document.getElementById('el-unit').value='MB';}document.getElementById('el-exp').value='';document.getElementById('el-fp').value=l.fingerprint||'chrome';document.getElementById('el-alpn').value=l.alpn||'';document.getElementById('el-port').value=l.port||443;document.getElementById('el-iplimit').value=l.ip_limit||0;if(!l.speed_limit_bytes){document.getElementById('el-speed').value='0';document.getElementById('el-speed-unit').value='MBIT';}else{document.getElementById('el-speed').value=(l.speed_limit_bytes*8/1024/1024).toFixed(2);document.getElementById('el-speed-unit').value='MBIT';}document.getElementById('el-category').value=l.category||'browsing';document.getElementById('el-bwsaver').checked=!!l.bandwidth_saver;openModal('modal-edit-link');}
function openEditLink(uuid){const l=allLinksList.find(x=>x.uuid===uuid);if(!l)return;document.getElementById('el-uuid').value=uuid;document.getElementById('el-label').value=l.label;document.getElementById('el-note').value=l.note||'';if(l.limit_bytes===0){document.getElementById('el-val').value='';document.getElementById('el-unit').value='GB';}else{document.getElementById('el-val').value=(l.limit_bytes/1024/1024).toFixed(0);document.getElementById('el-unit').value='MB';}document.getElementById('el-exp').value='';document.getElementById('el-fp').value=l.fingerprint||'chrome';document.getElementById('el-alpn').value=l.alpn||'';document.getElementById('el-port').value=l.port||443;document.getElementById('el-iplimit').value=l.ip_limit||0;if(!l.speed_limit_bytes){document.getElementById('el-speed').value='0';document.getElementById('el-speed-unit').value='MBIT';}else{document.getElementById('el-speed').value=(l.speed_limit_bytes*8/1024/1024).toFixed(2);document.getElementById('el-speed-unit').value='MBIT';}document.getElementById('el-category').value=l.category||'browsing';document.getElementById('el-bwsaver').checked=!!l.bandwidth_saver;openModal('modal-edit-link');}
async function saveEditLink(){const uuid=document.getElementById('el-uuid').value;const body={label:document.getElementById('el-label').value.trim(),note:document.getElementById('el-note').value.trim(),limit_value:document.getElementById('el-val').value||0,limit_unit:document.getElementById('el-unit').value,fingerprint:document.getElementById('el-fp').value||'chrome',alpn:document.getElementById('el-alpn').value.trim(),port:Number(document.getElementById('el-port').value)||443,ip_limit:Number(document.getElementById('el-iplimit').value)||0,speed_limit_value:Number(document.getElementById('el-speed').value)||0,speed_limit_unit:document.getElementById('el-speed-unit').value,category:document.getElementById('el-category').value||'browsing',bandwidth_saver:document.getElementById('el-bwsaver').checked};const exp=document.getElementById('el-exp').value;if(exp&&Number(exp)>0)body.expires_days=Number(exp);try{const r=await authF('/api/links/'+uuid,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});if(!r.ok)throw new Error();closeModal('modal-edit-link');toast('ویرایش شد ✓','ok');loadLinks();}catch(e){toast('خطا','err')}}
